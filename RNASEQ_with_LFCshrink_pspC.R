...
counts_initial <- read.table("RNASEQ_pspC.sf")
...
samples$generation <- as.factor(str_extract(samples$label, "^*[0-9a-z]+"))
...
dds$generation <- relevel(dds$generation, "11778a")

dds <- estimateSizeFactors(dds)
dds <- estimateDispersions(dds, fitType = 'local')
dds <- nbinomWaldTest(dds, betaPrior=FALSE)

res <- results(dds,contrast=c('generation', "11494a", "11778a"))
resLFC <- lfcShrink(dds, coef="generation_11494a_vs_11778a", type="apeglm")
...
write.csv(data,paste0('2.4_',name,'_Gene.csv'))
dataFC=data[data$log2FoldChange>1 | data$log2FoldChange < -1,]
write.csv(dataFC,paste0('2.5_',name,'_GeneFC.csv'))
